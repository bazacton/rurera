...
class QuestionsBankController extends Controller {
    ...
    public function index(Request $request) {
        ...
        $questions = $query->with([
                ...
                ->join('quiz_question_translations', 'quiz_question_translations.quizzes_question_id', '=', 'quizzes_questions.id')
                ->join('quizzes', 'quizzes.id', '=', 'quizzes_questions.quiz_id')
                ->join('webinars', 'webinars.id', '=', 'quizzes.webinar_id')
                ->join('webinar_translations', 'webinars.id', '=', 'webinar_translations.webinar_id')
                ->join('webinar_sub_chapters', 'webinar_sub_chapters.id', '=', 'quizzes_questions.sub_chapter_id')
                ->join('category_translations', 'category_translations.category_id', '=', 'webinars.category_id')
                ->select('quizzes_questions.*', 'webinar_sub_chapters.sub_chapter_title', 'quiz_question_translations.title', 'webinar_translations.title as subject_title', 'category_translations.title as category_title')
                ->paginate(50);
    ...
    public function store_question() {
                ...
                if (is_array($element_options) && array_key_exists('type', $element_options)) {
                    ...
                    if ($element_options['type'] != 'columns') {
                        ...
                        if ($field_type != 'checkbox' && $field_type != 'radio' && $field_type != 'sortable_quiz') {
                            ...
                            $attrib_arr = lmsParseTag($element_content, 'editor-field');
        ...
        $quizQuestion = QuizzesQuestion::create([
                    ...
                    'chapter_id' => isset($questionData['chapter_id']) ? $questionData['chapter_id'] : '',
