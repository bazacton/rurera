...
window.jQuery(document).ready(function($) {
	...
	// set the version (please do not change!)
	options.version = '1.6';
	...
	// check for WordPress
	if (window.svgAvatarsWPGlobals) { // WordPress plugin
		if (window.svgAvatarsWPGlobals.is_logged) {
			welcomeSlogan = translation.welcomeSloganLoggedIn;
		} else {
			welcomeSlogan = translation.welcomeSloganLoggedOut;
		}
		saveUrl = window.svgAvatarsWPGlobals.ajax_url;
		gravatarUrl = window.svgAvatarsWPGlobals.ajax_url;
		WPNonce = window.svgAvatarsWPGlobals.nonce;
	} else { // jQuery plugin
		welcomeSlogan = translation.welcomeSlogan;
		saveUrl = options.pathToFolder + 'php/save-ready-avatar.php';
		gravatarUrl = options.pathToFolder + 'php/send-to-gravatar.php';
		WPNonce = '';
	}
	...
	$('#svga-start-boys').on('click', function() {
		...
		$.ajax({
			...
			if (options.debug) { console.log('JSON fail response: ', response); }
	...
	function svgAvatars(gender, jsonData) {
		...
		function resetAvatar() {
			...
			if( avatar_color_settings.backs != undefined && avatar_color_settings.backs != 'undefined') {
				colorsStorage = {
					backs: avatar_color_settings.backs,
					humanbody: avatar_color_settings.humanbody,
					clothes: avatar_color_settings.clothes,
					hair: avatar_color_settings.hair,
					ears: avatar_color_settings.ears,
					faceshape: avatar_color_settings.faceshape,
					chinshadow: avatar_color_settings.chinshadow,
					facehighlight: avatar_color_settings.facehighlight,
					eyebrows: avatar_color_settings.eyebrows,
					eyesback: avatar_color_settings.eyesback,
					eyesfront: avatar_color_settings.eyesfront,
					eyesiris: avatar_color_settings.eyesiris,
					glasses: avatar_color_settings.glasses,
					mustache: avatar_color_settings.mustache,
					beard: avatar_color_settings.beard,
					mouth: avatar_color_settings.mouth,
				};
			}
			$('#svga-svgmain').empty();
			initGroups();
			...
			if( user_avatar_settings.backs != undefined && user_avatar_settings.backs != 'undefined') {
				$.each(user_avatar_settings, function (currentZone, shapesCounter) {
					$('#svga-elements-' + currentZone + '-' + shapesCounter).trigger('click');
					// Your code to process each key-value pair goes here
				});
			}
		...
		$('#svga-saveavatar').on('click', function() {
			...
			$.ajax({
				url: '/panel/users/update_avatar',//saveUrl,
				type: 'post',
				dataType: 'text',
				headers: {
				...
				data: {
					_ajax_nonce: WPNonce,
					action: 'svgAvatars_save_avatar',
					imgdata: img,
					avatarSettings: elementsStorage,
					avatarColorsSettings: colorsStorage,
					filename: filename
				},
				cache: false
			}).done(function(response) {
				if (options.debug) { console.log('Avatar save done response: ', response); }
				$('#svga-message-text').html('Avatar Generated Successfully!').removeClass('svga-error');
				$('#svga-work-overlay').fadeIn('fast');
				$('#svga-message').fadeIn('fast');
				window.location.href = '/panel/setting';
			}).fail(function(response) {
				if (options.debug) { console.log('Avatar save fail response: ', response); }
				overlayMessage(translation.alertErrorCommon);
			});
		});
		function overlayMessage(error) {
			$('#svga-message-text').html(error).addClass('svga-error');
			$('#svga-work-overlay').fadeIn('fast');
			$('#svga-message').fadeIn('fast');
		}
		...
		$('#svga-gravatar-ok').on('click', function() {
			...
			$.ajax({
				...
				data: {
					_ajax_nonce: WPNonce,
					action: 'svgAvatars_send_gravatar',
					imgdata: img,
					datastring1: email,
					datastring2: pass,
					rating: rating
				...
				if (options.debug) { console.log('Gravatar done response: ', response); }
				switch (response) {
					case 'gravatar_success': {
					...
					case 'gravatar_email_fail': {
						overlayGravatarMessage(translation.alertErrorEmailFail);
						break;
					}
					case 'gravatar_password_fail': {
						overlayGravatarMessage(translation.alertErrorPasswordFail);
						break;
					}
					case 'gravatar_faultcode-8': {
						$('#svga-loader').hide();
						overlayMessage(translation.alertErrorFaultCode8);
						break;
					}
					case 'gravatar_faultcode-9': {
						overlayGravatarMessage(translation.alertErrorFaultCode9);
						break;
					}
					default: {
						overlayGravatarMessage(translation.alertErrorCommon);
						break;
					}
				}
			}).fail(function(response) {
				if (options.debug) { console.log('Gravatar fail response: ', response); }
				overlayGravatarMessage(translation.alertErrorCommon);
			});
		});
		function overlayGravatarMessage(error) {
			$('#svga-loader').hide();
		...
		$('#svga-png-one').on('click', function() {
			...
			$('#svga-canvas').attr({width: 200, height: 200});
			$.ajax({
				url: options.pathToFolder + "php/temp-avatar-save.php",
				type: "post",
				dataType: 'text',
				data: {
					imgdata: img,
				...
				if (options.debug) { console.log('PNG 1 done response: ', response); }
				if (response === 'saved') {
					$('#svga-message-text').removeClass('svga-error');
				...
				if (options.debug) { console.log('PNG 1 fail response: ', response); }
				overlayMessage(translation.alertErrorCommon);
			});
		});
		...
		$('#svga-png-two').on('click', function() {
			...
			$.ajax({
				...
				if (options.debug) { console.log('PNG 2 done response: ', response); }
				if (response === 'saved') {
					$('#svga-message-text').removeClass('svga-error');
					getFile(filename, 'png');
					$('#svga-work-overlay').fadeOut('fast');
				...
				if (options.debug) { console.log('PNG 2 fail response: ', response); }
				overlayMessage(translation.alertErrorCommon);
			});
		});
		...
		if (iOS) {
			$('#svga-downloadavatar').on('click', function() {
				...
				$.ajax({
					...
					if (options.debug) { console.log('PNG (iOS) done response: ', response); }
					if (response === 'saved') {
						$('#svga-message-text').removeClass('svga-error');
						var iosImg = $('<img />')
							.on('error', function() {
								$( this ).hide();
								overlayMessage(translation.alertErrorImageCreate);
							})
					...
					if (options.debug) { console.log('PNG (iOS) fail response: ', response); }
					overlayMessage(translation.alertErrorCommon);
				});
			});
		}
		...
		if (Win8tablet) {
			$('#svga-downloadavatar').on('click', function() {
				...
				$.ajax({
					...
					if (options.debug) { console.log('PNG (Win8tablet) done response: ', response); }
					if (response === 'saved') {
						$('#svga-message-text').removeClass('svga-error');
						getFile(filename, 'png');
						$('#svga-work-overlay').fadeOut('fast');
					...
					if (options.debug) { console.log('PNG (Win8tablet) fail response: ', response); }
					overlayMessage(translation.alertErrorCommon);
				});
			});
		}
		...
		$('#svga-svgfile').on('click', function() {
			...
			$.ajax({
				...
				if (options.debug) { console.log('SVG done response: ', response); }
				if (response === 'saved') {
					$('#svga-message-text').removeClass('svga-error');
				...
				if (options.debug) { console.log('SVG fail response: ', response); }
				overlayMessage(translation.alertErrorCommon);
			});
		});
		...
		$('#svga-shareavatar').on('click', function() {
			...
			$.ajax({
				...
				if (options.debug) { console.log('Share done response: ', response); }
				if (response === 'saved') {
					$('#svga-message-text').removeClass('svga-error');
					var shareImg = $('<img />')
						.on('error', function() {
				...
				if (options.debug) { console.log('Share fail response: ', response); }
				overlayMessage(translation.alertErrorCommon);
			});
		});
		...
		//service function to force download a previously saved avatar from a temporary folder
		function getFile(tempFile, fileType) {
			var fD = new FileDownloader(options.pathToFolder + 'php/temp-avatar-download.php');
		...
		function initZoom() {
			if (options.zooming < 0) {
				for (i = -1; i >= options.zooming && i >= -3; i--) {
					SVG.get('svga-group-humanwrap').svgaScaleDown();
				}
			} else if (options.zooming > 0) {
				for (i = 1; i <= options.zooming && i <= 3; i++) {
					SVG.get('svga-group-humanwrap').svgaScaleUp();
				}
			}
		...
		$('#svga-colors-faceshape > div:nth-child(1)').trigger('click');
		//initial zooming of full avatar
		initZoom();
