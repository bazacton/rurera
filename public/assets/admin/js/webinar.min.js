                ...
                if (itemId) {
                    ...
                    $.get(adminPanelPrefix + '/chapters/' + itemId + '?locale=' + locale, function (result) {
                        if (result && result.chapter) {
                            ...
                            modal.find('.js-ajax-chapter_slug').val(result.chapter.chapter_slug);
                            modal.find('.js-ajax-challenge_title').val(result.chapter.challenge_title);
                            modal.find('.js-ajax-challenge_image').val(result.chapter.challenge_image);
                            modal.find('.js-ajax-challenge_background_color').val(result.chapter.challenge_background_color);
                            modal.find('.js-ajax-challenge_border_color').val(result.chapter.challenge_border_color);
                            modal.find('.js-ajax-custom_link').val(result.chapter.custom_link);
                            modal.find('.js-ajax-challenge_quiz').html(result.challenge_quiz_layout);
                            handleQuizMultiSelect2('search-quiz-field-select2', '/admin/quizzes/search_quiz', ['class', 'course', 'subject', 'title']);
    ...
    $('body').on('click', '.js-add-course-content-btn, .add-new-interactive-file-btn', function (e) {
        ...
        switch (type) {
            ...
            case 'text_lesson':
                ...
                $('.search-sub_chapter-select2').select2({
                    placeholder: $(this).data('placeholder'),
                    minimumInputLength: 3,
                    allowClear: true,
                    ajax: {
                        url: '/admin/webinars/search_sub_chapter',
                        dataType: 'json',
                        type: "POST",
                        quietMillis: 50,
            ...
            case 'quiz':
                ...
                newQuizForm.find('.chapter_id_field').val(chapter_id);
                newQuizForm.find('.sub_chapter_id_field').val(subchapter_id);

                html = newQuizForm.html();
                html = html.replace(/record/g, key);
                $(contentTagId).prepend(html);
                console.log('chapter_id-'+ chapter_id);
                console.log('subchapter_id-'+ subchapter_id);

                $('.search-quiz-select2').select2({
                    placeholder: $(this).data('placeholder'),
                    minimumInputLength: 3,
                    allowClear: true,
                    ajax: {
                        url: '/admin/quizzes/search_quiz',
                        dataType: 'json',
                        type: "POST",
                        quietMillis: 50,
                        data: function (params) {
            ...
            case 'sub':
                var newSubForm = $('#newSubForm');
                newSubForm.find('.chapter-input').val(chapterId);
                html = newSubForm.html();
                html = html.replace(/record/g, key);
                $(contentTagId).prepend(html);
                break;
        }
